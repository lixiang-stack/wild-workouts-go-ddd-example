...
func (d TrainingsFirestoreRepository) FindTrainingsForUser(ctx context.Context, userUUID string) ([]query.Training, error) {
	...
	return d.trainingModelsToQuery(iter)
}

func (d TrainingsFirestoreRepository) AllTrainings(ctx context.Context) ([]query.Training, error) {
	query := d.trainingsCollection().Query.Where("Time", ">=", time.Now().Add(-time.Hour*24)).
		Where("Canceled", "==", false)
